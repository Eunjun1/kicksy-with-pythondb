...
class _HqModelDetailState extends State<HqModelDetail> {
  ...
  getData() async {
    await getJSONDataModel(value[1]);
    await getJSONDataProd(value[1]);
    await getJSONDataImg(value[0]);
    modelImage =
        await 'http://127.0.0.1:8000/image/view/name=${value[0]}&img_num=0';
  }
                  ...
                  : SingleChildScrollView(
                    child: Column(
                      children: [
                        ...
                        Text(
                          '등록 날짜 : ${product[0]['registration'].substring(0, 10)}',
                        ),
                        ElevatedButton(
                          onPressed: () => Get.back(),
                          child: Text('확인'),
